...
class MyApp(ShowBase):
    ...
    def setupScene(self): ## snailCase for entire project
        ...
        self.sun     = spaceJamClasses.Sun(self.loader, "Assets/Planets/protoPlanet.x", self.render, "Sun", "Assets/Planets/Textures/Sun.jpg",            1,  1000,   1,  400, self.render) 
        ...
        self.planets = [self.planet1, self.planet2, self.planet3, self.planet4, self.planet5, self.planet6, self.sun]
    ...
    def setRotations(self, rot):
        for i in range(len(self.planets)):
            name = self.planets[i].nodeName
            print("rotating " + str(name))
            x = self.planets[i].modelNode.getPos()[0]
            y = self.planets[i].modelNode.getPos()[1]
            z = self.planets[i].modelNode.getPos()[2]
            h = self.planets[i].modelNode.getHpr()[0]
            p = self.planets[i].modelNode.getHpr()[1]
            r = self.planets[i].modelNode.getHpr()[2]
            sunH = self.sun.modelNode.getHpr()[0]
            sunP = self.sun.modelNode.getHpr()[1]
            sunR = self.sun.modelNode.getHpr()[2]
            relx = x * self.sun.modelNode.getPos()[0]
            rely = y * self.sun.modelNode.getPos()[1]
            relz = z * self.sun.modelNode.getPos()[2]
            
            self.rotation = self.planets[i].modelNode.hprInterval(80, (360*rot,360*rot,0*rot))
            #self.rotation = self.planets[i].modelNode.hprInterval(80, (360*rot,360*rot,0*rot))
            
            #self.rotation.pause()
            #if self.planets[i].nodeName != "Sun":
                #orbit = self.planets[i].modelNode.posInterval(100, (relx,rely,relz))
                #orbit.loop()
            #rotation.start()
            self.rotation.loop()
